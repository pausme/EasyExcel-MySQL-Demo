...
| [接口扁平化测试用例.xlsx](接口扁平化测试用例.xlsx) | 全量用例矩阵（131 条）+ 接口参数列表 + 执行结果，三个工作表 |
...
| [../performance-report.md](../performance-report.md) | 百万级导入/导出性能压测与调优报告（TODO #6 交付） |
...
# 2. 启动被测应用（使用真实环境参数，端口 18088）
java -DSERVER_PORT=18088 -DMYSQL_URL=... -jar target/demo-0.0.1-SNAPSHOT.jar
...
- 用例矩阵：**131 条**（Excel 32 / 任务中心 21 / 报表运行控制 30 / 文件上传中心 48），覆盖 32 个接口
- 标准环境权威轮（R7，Bearer 鉴权）：**76 用例 / 72 通过 / 4 失败**（真实缺陷仅 F-02 三项 500→400 映射；1 项为空库前置条件失真的用例设计问题），见 [测试执行记录.md](测试执行记录.md)
