...
- 标准环境全量回归（R22，学生查询+游标分页版，2026-08-22 10:15）：**77 用例 / 77 通过 / 0 失败**——学生分页/游标分页定向实测（过滤精确命中、参数 400 含 ERR-02 字段、401）；分页越界 400 行为变更已同步；见 [测试执行记录.md](测试执行记录.md) §3.25
...
- 性能（标准环境，R10/R25）：1M 导出 ~23,300 行/s（XLSX）；**3M 导出 90,738 行/s（ZIP_CSV_PARTS，R25）**；100k 导入 4,511 行/s；1M 导入成功（221s / 4,521 行/s）——详见 [../performance-report.md](../performance-report.md) §5.5
- 历史轮次（R1–R6）数据已全部作废
