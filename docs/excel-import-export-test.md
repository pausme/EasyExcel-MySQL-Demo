...
本文用于说明 Excel 导入导出模块的测试范围、执行方式、验收标准和当前测试结论。更完整的接口扁平化执行记录见 [docs/test/测试执行记录.md](test/测试执行记录.md)，性能复盘见 [docs/performance-report.md](performance-report.md)。
...
- 异步导入：模板下载、源文件上传 MinIO、后台解析、暂存表写入、校验、全量原子合并。
...
| --- | --- |
| `mvn test` | 40 用例全部通过 |
| 接口扁平化测试 | R7 历史记录为 76 用例，72 通过 / 4 失败；当前脚本已加固环境相关误判 |
| 已修复缺陷 | F-02：框架层参数异常已本地回归修复，返回 400/415；导出超限和取消竞态按任务终态动态断言 |
| 1M 导出 | 3/3 成功，平均约 23,317 行/s |
| 100k 导入 | 3/3 成功，平均约 3,908 行/s |
| 1M 导入 | 当前小规格标准环境不可直接执行，已确认 OOM 和长事务风险 |
