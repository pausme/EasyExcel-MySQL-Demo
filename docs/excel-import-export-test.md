...
验证学生数据的异步导出、MinIO 文件保存、签名下载地址、Excel 全量原子导入，以及重复学号拦截行为。
...
| I-05 | 同一文件出现重复 `student_no` | 导入任务失败，正式表不发生本次导入变更 |
| I-06 | 第 N 批暂存或最终合并发生数据库异常 | 导入任务失败，正式表不发生本次导入变更，暂存数据被清理或等待过期清理 |
...
| P-03 | 112000 条回导 | 总耗时、批次数、数据库连接数 | `imported=112000`，总数不因重复学号而增长，失败时正式表不发生本次导入变更 |
...
| --- | --- | --- | --- | --- | --- |
| 2026-08-04 17:50 | 4 个导入 worker，单任务并发，2000 行/批 | P-03 | 通过 | 1,000,000 行，`imported=1000000`，`batchCount=500`，`elapsedMs=42718`；未发现失败、死锁、重试或超时 | |
| 2026-08-04 17:52 | 单任务导出，单 Sheet | P-01 | 通过 | `taskId=0db66ee470bd4690a4a64534530f19c3`，`total=1000000`，`exported=1000000`，`sheetCount=1`，`elapsedMs=63806` | |
| 2026-08-04 18:05 | 6 个导入 worker，单任务并发，2000 行/批 | P-03 | 通过 | 1,000,000 行，`imported=1000000`，`batchCount=500`，`elapsedMs=33021`；未发现失败、死锁、重试或超时 | |
| 2026-08-05 10:18 | 16 个导入 worker，单任务并发，2000 行/批 | P-03 | 通过 | 1,000,000 行，`imported=1000000`，`batchCount=500`，`elapsedMs=15671`；未发现失败、死锁、重试或超时 | |
| 2026-08-05 10:35 | 8 个导入 worker，单任务并发，2000 行/批 | P-03 | 通过 | 1,000,000 行，`imported=1000000`，`batchCount=500`，`elapsedMs=34265`；未发现失败、死锁、重试或超时 | |
