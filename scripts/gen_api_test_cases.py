...
# ============================ 鉴权（部署版已开启） ============================
SEC = "鉴权"
add(SEC, "GET /api/excel/count（鉴权探针）", [
    ("SEC-401-01", "P0", "异常", "不携带 Authorization 头", "GET", "/api/excel/count", "（无）", "",
     "401", "返回统一错误体 COMMON_UNAUTHORIZED“请先登录”", "R7 实测通过"),
    ("SEC-401-02", "P1", "异常", "携带无效 Token", "GET", "/api/excel/count", "Authorization: Bearer invalid-token", "",
     "401", "无效 Token 同样 401，不泄露有效 Token 形态", ""),
    ("SEC-200-01", "P0", "正常", "携带有效用户 Token", "GET", "/api/excel/count", "Bearer 用户Token（user-1）", "",
     "200", "正常返回业务数据，归属为 user-1", ""),
])
...
add(EXC, "GET /api/excel/count", [
    ("EXC-COUNT-01", "P0", "正常", "系统已启动", "GET", "/api/excel/count", "", "",
     "200", "返回 JSON，含 count 字段（当前学生总数，可为 0）", "需 Bearer Token（部署版已开启鉴权）"),
...
add(EXC, "POST /api/excel/seed/{count}", [
    ...
    ("EXC-SEED-03", "P2", "异常", "无", "POST", "/api/excel/seed/abc", "", "",
     "400", "路径变量非整数 → 400", "⚠️F-02：当前版本实际返回 500，待修复"),
])
add(EXC, "POST /api/excel/export", [
    ("EXC-EXP-01", "P0", "正常", "已存在学生数据", "POST", "/api/excel/export", "Bearer 用户Token（user-1）", "",
...
add(EXC, "GET /api/excel/export/{taskId}", [
    ("EXC-EXPS-01", "P0", "正常", "已提交导出任务", "GET", "/api/excel/export/{taskId}", "Bearer 用户Token（user-1）", "",
     "200", "返回任务详情；轮询可见 QUEUED→RUNNING→SUCCESS；SUCCESS 时 exported=total、sheetCount=1", ""),
    ("EXC-EXPS-02", "P1", "异常", "taskId 不存在", "GET", "/api/excel/export/not-exist", "Bearer 用户Token（user-1）", "",
     "404", "“导出任务不存在”", ""),
    ("EXC-EXPS-03", "P1", "权限", "用其他用户查询他人任务", "GET", "/api/excel/export/{taskId}", "Bearer 管理员Token（admin，另一身份）", "",
     "404", "归属不匹配返回 404，不泄露任务存在性", "越权防探测"),
])
add(EXC, "GET /api/excel/export/{taskId}/download", [
    ("EXC-EXPD-01", "P0", "正常", "任务 status=SUCCESS", "GET", "/api/excel/export/{taskId}/download", "Bearer 用户Token（user-1）", "",
     "302", "返回 302 + Location（MinIO 签名下载地址）", ""),
    ("EXC-EXPD-02", "P1", "业务规则", "任务尚未 SUCCESS（QUEUED/RUNNING）", "GET", "/api/excel/export/{taskId}/download", "Bearer 用户Token（user-1）", "",
     "409", "“导出任务尚未完成”", ""),
    ("EXC-EXPD-03", "P1", "异常", "任务成功但导出对象缺失", "GET", "/api/excel/export/{taskId}/download", "Bearer 用户Token（user-1）", "",
     "404", "“导出文件不存在”", "对象被生命周期清理"),
    ("EXC-EXPD-04", "P1", "权限", "非归属用户下载", "GET", "/api/excel/export/{taskId}/download", "Bearer 管理员Token（admin，另一身份）", "",
...
add(EXC, "POST /api/excel/import", [
    ("EXC-IMP-01", "P0", "正常", "准备好合法 Excel（含新学号）", "POST", "/api/excel/import", "Bearer 用户Token（user-1）",
    ...
    ("EXC-IMP-03", "P1", "异常", "未携带 file 参数", "POST", "/api/excel/import", "", "（无 file）",
     "400/415", "缺失 file part → 400/415", "⚠️F-02：当前版本实际返回 500，待修复"),
...
add(EXC, "GET /api/excel/import/{taskId}", [
    ("EXC-IMPS-01", "P0", "正常", "已提交导入任务", "GET", "/api/excel/import/{taskId}", "Bearer 用户Token（user-1）", "",
     "200", "返回 ImportTaskResponse；进度 completed/total；失败时含 errorMessage 与错误文件信息", ""),
    ("EXC-IMPS-02", "P1", "异常", "taskId 不存在", "GET", "/api/excel/import/not-exist", "Bearer 用户Token（user-1）", "",
     "404", "“导入任务不存在”", ""),
    ("EXC-IMPS-03", "P1", "权限", "非归属用户查询", "GET", "/api/excel/import/{taskId}", "Bearer 管理员Token（admin，另一身份）", "",
     "404", "归属不匹配返回 404", ""),
])
add(EXC, "GET /api/excel/import/{taskId}/error-file", [
    ("EXC-ERRF-01", "P1", "正常", "导入存在校验失败行", "GET", "/api/excel/import/{taskId}/error-file", "Bearer 用户Token（user-1）", "",
     "302", "返回 302 + Location（错误明细 Excel 签名地址）", ""),
    ("EXC-ERRF-02", "P1", "异常", "导入无错误（无错误文件）", "GET", "/api/excel/import/{taskId}/error-file", "Bearer 用户Token（user-1）", "",
     "404", "“导入错误明细文件不存在”", ""),
    ("EXC-ERRF-03", "P1", "权限", "非归属用户", "GET", "/api/excel/import/{taskId}/error-file", "Bearer 管理员Token（admin，另一身份）", "",
     "404", "归属校验返回 404", ""),
])
...
add(TSK, "GET /api/tasks/{taskId}", [
    ("TSK-GET-01", "P0", "正常", "存在归属任务", "GET", "/api/tasks/{taskId}", "Bearer 用户Token（user-1）", "",
     "200", "返回 AsyncTaskResponse（taskId/type/status/progress/retryCount 等）", ""),
    ("TSK-GET-02", "P1", "异常", "taskId 不存在", "GET", "/api/tasks/not-exist", "Bearer 用户Token（user-1）", "",
     "404", "“任务不存在”", ""),
    ("TSK-GET-03", "P1", "权限", "查询他人任务", "GET", "/api/tasks/{taskId}", "Bearer 管理员Token（admin，另一身份）", "",
     "404", "归属不匹配返回 404", ""),
])
add(TSK, "POST /api/tasks/page", [
    ("TSK-PAGE-01", "P0", "正常", "存在任务", "POST", "/api/tasks/page", "Bearer 用户Token（user-1）",
    ...
    ("TSK-PAGE-03", "P2", "边界", "无", "POST", "/api/tasks/page", "Bearer 用户Token（user-1）",
     '{"pageNo":0,"pageSize":999}', "200", "pageNo<1 → 1；pageSize 受 max-page-size(100) 限制", ""),
    ("TSK-PAGE-04", "P1", "正常", "存在不同类型/状态任务", "POST", "/api/tasks/page", "Bearer 用户Token（user-1）",
     '{"taskType":"IMPORT","status":"SUCCESS"}', "200", "按 taskType 与 status 过滤", ""),
    ("TSK-PAGE-05", "P2", "异常", "无", "POST", "/api/tasks/page", "Bearer 用户Token（user-1）",
     '{"status":"GARBAGE"}', "400", "统一异常处理将非法枚举转为 400 COMMON_PARAM_ERROR（2026-08-14 已验证）", "已由 TODO#8 修复"),
    ("TSK-PAGE-06", "P2", "异常", "无", "POST", "/api/tasks/page", "Bearer 用户Token（user-1）",
     '{"taskType":"GARBAGE"}', "400", "非法任务类型枚举→400 COMMON_PARAM_ERROR", "已由 TODO#8 修复"),
])
add(TSK, "POST /api/tasks/{taskId}/cancel", [
    ("TSK-CANCEL-01", "P1", "正常", "任务处于 CREATED/RUNNING", "POST", "/api/tasks/{taskId}/cancel", "Bearer 用户Token（user-1）", "",
     "200", "canceled=true，任务变为 CANCELED", "前置：任务需仍在排队/执行中（小数据量导出可能秒级完成导致 canceled=false）"),
    ("TSK-CANCEL-02", "P2", "业务规则", "任务已是 CANCELED", "POST", "/api/tasks/{taskId}/cancel", "Bearer 用户Token（user-1）", "",
     "200", "canceled=true（终态 CANCELED 返回 true）", ""),
    ("TSK-CANCEL-03", "P2", "业务规则", "任务已 SUCCESS/FAILED/EXPIRED", "POST", "/api/tasks/{taskId}/cancel", "Bearer 用户Token（user-1）", "",
     "200", "canceled=false（终态非 CANCELED 不再变更）", ""),
    ("TSK-CANCEL-04", "P1", "异常", "taskId 不存在", "POST", "/api/tasks/not-exist/cancel", "Bearer 用户Token（user-1）", "",
     "404", "“任务不存在”", ""),
    ("TSK-CANCEL-05", "P1", "权限", "取消他人任务", "POST", "/api/tasks/{taskId}/cancel", "Bearer 管理员Token（admin，另一身份）", "",
     "404", "归属校验返回 404", ""),
])
add(TSK, "POST /api/tasks/{taskId}/retry", [
    ("TSK-RETRY-01", "P1", "正常", "任务 FAILED 且 retryCount<max", "POST", "/api/tasks/{taskId}/retry", "Bearer 用户Token（user-1）", "",
     "200", "任务重置为 CREATED，retryCount+1", ""),
    ("TSK-RETRY-02", "P2", "业务规则", "任务 CANCELED/EXPIRED", "POST", "/api/tasks/{taskId}/retry", "Bearer 用户Token（user-1）", "",
     "200", "允许重试，重置为 CREATED", "前置：任务需仍处 CANCELED/EXPIRED（若已被重新执行为 SUCCESS 则应 409）"),
    ("TSK-RETRY-03", "P1", "业务规则", "任务 SUCCESS/RUNNING/CREATED", "POST", "/api/tasks/{taskId}/retry", "Bearer 用户Token（user-1）", "",
     "409", "“当前任务状态不允许重试”", ""),
    ("TSK-RETRY-04", "P1", "业务规则", "retryCount>=maxRetryCount", "POST", "/api/tasks/{taskId}/retry", "Bearer 用户Token（user-1）", "",
     "409", "“任务重试次数已达上限”", ""),
    ("TSK-RETRY-05", "P2", "业务规则", "任务类型无重试处理器", "POST", "/api/tasks/{taskId}/retry", "Bearer 用户Token（user-1）", "",
     "409", "“当前任务类型暂不支持重试”", ""),
    ("TSK-RETRY-06", "P1", "异常", "taskId 不存在", "POST", "/api/tasks/not-exist/retry", "Bearer 用户Token（user-1）", "",
     "404", "“任务不存在”", ""),
    ("TSK-RETRY-07", "P1", "权限", "重试他人任务", "POST", "/api/tasks/{taskId}/retry", "Bearer 管理员Token（admin，另一身份）", "",
     "404", "归属校验返回 404", ""),
])
...
add(RPT, "POST /api/report/student-runs/page", [
    ("RPT-PAGE-01", "P0", "正常", "存在运行控制", "POST", "/api/report/student-runs/page", "Bearer 用户Token（user-1）",
     '{"pageNo":1,"pageSize":10}', "200", "返回当前归属运行控制列表", ""),
    ("RPT-PAGE-02", "P2", "正常", "按名称过滤", "POST", "/api/report/student-runs/page", "Bearer 用户Token（user-1）",
     '{"runName":"学期"}', "200", "按 runName 模糊过滤", ""),
    ("RPT-PAGE-03", "P2", "边界", "无", "POST", "/api/report/student-runs/page", "Bearer 用户Token（user-1）",
     '{"pageNo":0,"pageSize":999}', "200", "pageNo=1；pageSize 受 100 限制", ""),
    ("RPT-PAGE-04", "P2", "异常", "无", "POST", "/api/report/student-runs/page", "Bearer 用户Token（user-1）",
     '{"status":"GARBAGE"}', "400", "非法状态枚举→400 COMMON_PARAM_ERROR", "已由 TODO#8 修复"),
])
add(RPT, "POST /api/report/student-runs/create", [
    ("RPT-CRT-01", "P0", "正常", "无", "POST", "/api/report/student-runs/create", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R001","runName":"2026春季","minAge":18,"maxAge":25}', "200",
     "返回运行控制详情，status=NORMAL，生成 runId", ""),
    ("RPT-CRT-02", "P1", "异常", "无", "POST", "/api/report/student-runs/create", "Bearer 用户Token（user-1）",
     '{}', "400", "“运行控制编码不能为空/运行控制名称不能为空”", "缺必填"),
    ("RPT-CRT-03", "P1", "异常", "已存在编码 R001", "POST", "/api/report/student-runs/create", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R001","runName":"dup"}', "400", "“运行控制编码已存在”", "唯一约束"),
    ("RPT-CRT-04", "P2", "边界", "无", "POST", "/api/report/student-runs/create", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R002","runName":"x","minAge":-1}', "400", "“最小年龄必须在0到150之间”", ""),
    ("RPT-CRT-05", "P2", "边界", "无", "POST", "/api/report/student-runs/create", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R003","runName":"x","maxAge":200}', "400", "“最大年龄必须在0到150之间”", ""),
    ("RPT-CRT-06", "P2", "边界", "无", "POST", "/api/report/student-runs/create", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R004","runName":"x","minAge":30,"maxAge":20}', "400", "“最小年龄不能大于最大年龄”", ""),
    ("RPT-CRT-07", "P2", "边界", "无", "POST", "/api/report/student-runs/create", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R005","runName":"x"}', "200", "可选条件全空也可创建", "仅必填项"),
])
add(RPT, "GET /api/report/student-runs/{runId}", [
    ("RPT-GET-01", "P0", "正常", "存在归属运行控制", "GET", "/api/report/student-runs/{runId}", "Bearer 用户Token（user-1）", "",
     "200", "返回运行控制详情", ""),
    ("RPT-GET-02", "P1", "异常", "runId 不存在", "GET", "/api/report/student-runs/not-exist", "Bearer 用户Token（user-1）", "",
     "404", "“运行控制不存在”", ""),
    ("RPT-GET-03", "P1", "权限", "查询他人运行控制", "GET", "/api/report/student-runs/{runId}", "Bearer 管理员Token（admin，另一身份）", "",
     "404", "归属不匹配返回 404", ""),
    ("RPT-GET-04", "P2", "业务规则", "运行控制已逻辑删除", "GET", "/api/report/student-runs/{runId}", "Bearer 用户Token（user-1）", "",
     "404", "已删除对当前用户不可见", ""),
])
add(RPT, "POST /api/report/student-runs/{runId}/update", [
    ("RPT-UPD-01", "P0", "正常", "存在归属运行控制", "POST", "/api/report/student-runs/{runId}/update", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R001","runName":"2026春季改","className":"一班"}', "200", "返回更新后详情", ""),
    ("RPT-UPD-02", "P2", "正常", "改为自身原编码", "POST", "/api/report/student-runs/{runId}/update", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R001","runName":"x"}', "200", "编码与自身相同不判重，更新成功", ""),
    ("RPT-UPD-03", "P1", "业务规则", "编码改为他人已用编码", "POST", "/api/report/student-runs/{runId}/update", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R002","runName":"x"}', "400", "“运行控制编码已存在”", ""),
    ("RPT-UPD-04", "P1", "异常", "runId 不存在", "POST", "/api/report/student-runs/not-exist/update", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R","runName":"x"}', "404", "“运行控制不存在”", ""),
    ("RPT-UPD-05", "P2", "边界", "存在归属运行控制", "POST", "/api/report/student-runs/{runId}/update", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R001","runName":"x","minAge":40,"maxAge":30}', "400", "“最小年龄不能大于最大年龄”", ""),
])
add(RPT, "POST /api/report/student-runs/{runId}/delete", [
    ("RPT-DEL-01", "P0", "正常", "存在归属运行控制", "POST", "/api/report/student-runs/{runId}/delete", "Bearer 用户Token（user-1）", "",
     "200", "deleted=true；逻辑删除，deleted 写入本行 id", ""),
    ("RPT-DEL-02", "P1", "异常", "runId 不存在", "POST", "/api/report/student-runs/not-exist/delete", "Bearer 用户Token（user-1）", "",
     "404", "“运行控制不存在”", ""),
    ("RPT-DEL-03", "P1", "权限", "删除他人运行控制", "POST", "/api/report/student-runs/{runId}/delete", "Bearer 管理员Token（admin，另一身份）", "",
     "404", "归属校验返回 404", ""),
    ("RPT-DEL-04", "P2", "业务规则", "删除后重建同编码", "POST", "/api/report/student-runs/create", "Bearer 用户Token（user-1）",
     '{"runControlCode":"R001","runName":"重建"}', "200", "删除后同用户可重建相同编码", ""),
])
add(RPT, "POST /api/report/student-runs/{runId}/run", [
    ("RPT-RUN-01", "P0", "正常", "存在归属运行控制", "POST", "/api/report/student-runs/{runId}/run", "Bearer 用户Token（user-1）", "",
     "200", "创建导出任务，返回 ExportTaskResponse；任务 businessKey=runId", ""),
    ("RPT-RUN-02", "P1", "异常", "runId 不存在", "POST", "/api/report/student-runs/not-exist/run", "Bearer 用户Token（user-1）", "",
     "404", "“运行控制不存在”", ""),
    ("RPT-RUN-03", "P1", "权限", "运行他人运行控制", "POST", "/api/report/student-runs/{runId}/run", "Bearer 管理员Token（admin，另一身份）", "",
     "404", "归属校验返回 404", ""),
])
add(RPT, "POST /api/report/student-runs/{runId}/tasks", [
    ("RPT-TASKS-01", "P1", "正常", "该运行控制已 run 过", "POST", "/api/report/student-runs/{runId}/tasks", "Bearer 用户Token（user-1）",
     '{"pageNo":1,"pageSize":10}', "200", "按 businessKey=runId 分页返回历史导出任务", ""),
    ("RPT-TASKS-02", "P1", "异常", "runId 不存在", "POST", "/api/report/student-runs/not-exist/tasks", "Bearer 用户Token（user-1）",
     '{}', "404", "“运行控制不存在”", ""),
    ("RPT-TASKS-03", "P2", "权限", "查询他人运行控制任务", "POST", "/api/report/student-runs/{runId}/tasks", "Bearer 管理员Token（admin，另一身份）",
     '{}', "404", "归属校验返回 404", ""),
])
...
# ============================== 接口参数列表 ==============================
PARAMS = [
    ("Excel 导入导出", "查询学生总数", "GET", "/api/excel/count", "—", "—", "—", "{count}", "200", "Bearer", ""),
    ("Excel 导入导出", "生成演示数据", "POST", "/api/excel/seed/{count}", "count:int", "—", "—", "{inserted,count,elapsedMs}", "200/400", "Bearer", ""),
    ("Excel 导入导出", "提交导出任务", "POST", "/api/excel/export", "—", "X-User-Id?", "—", "ExportTaskResponse", "200", "Bearer（身份归属）", "异步"),
    ("Excel 导入导出", "查询导出任务状态", "GET", "/api/excel/export/{taskId}", "taskId", "X-User-Id?", "—", "ExportTaskResponse", "200/404", "Bearer（身份归属）", ""),
    ("Excel 导入导出", "下载导出文件", "GET", "/api/excel/export/{taskId}/download", "taskId", "X-User-Id?", "—", "302 Location", "302/404/409", "Bearer（身份归属）", ""),
    ("Excel 导入导出", "下载导入模板", "GET", "/api/excel/template", "—", "—", "—", "xlsx 流", "200", "无", ""),
    ("Excel 导入导出", "提交导入任务", "POST", "/api/excel/import", "—", "X-User-Id?", "multipart file", "ImportTaskResponse", "200/400/413", "Bearer（身份归属）", "异步"),
    ("Excel 导入导出", "查询导入任务状态", "GET", "/api/excel/import/{taskId}", "taskId", "X-User-Id?", "—", "ImportTaskResponse", "200/404", "Bearer（身份归属）", ""),
    ("Excel 导入导出", "下载导入错误明细", "GET", "/api/excel/import/{taskId}/error-file", "taskId", "X-User-Id?", "—", "302 Location", "302/404", "Bearer（身份归属）", ""),
    ("异步任务中心", "查询任务详情", "GET", "/api/tasks/{taskId}", "taskId", "X-User-Id?", "—", "AsyncTaskResponse", "200/404", "Bearer（身份归属）", ""),
    ("异步任务中心", "分页查询任务", "POST", "/api/tasks/page", "—", "X-User-Id?", "{pageNo,pageSize,taskType?,status?}", "AsyncTaskPageResponse", "200/400", "Bearer（身份归属）", ""),
    ("异步任务中心", "取消任务", "POST", "/api/tasks/{taskId}/cancel", "taskId", "X-User-Id?", "—", "{taskId,canceled}", "200/404", "Bearer（身份归属）", ""),
    ("异步任务中心", "重试任务", "POST", "/api/tasks/{taskId}/retry", "taskId", "X-User-Id?", "—", "AsyncTaskResponse", "200/404/409", "Bearer（身份归属）", ""),
    ("报表运行控制", "分页查询运行控制", "POST", "/api/report/student-runs/page", "—", "X-User-Id?", "{pageNo,pageSize,runName?,status?}", "StudentReportRunPageResponse", "200/400", "Bearer（身份归属）", ""),
    ("报表运行控制", "创建运行控制", "POST", "/api/report/student-runs/create", "—", "X-User-Id?", "StudentReportRunCreateRequest", "StudentReportRunResponse", "200/400", "Bearer（身份归属）", ""),
    ("报表运行控制", "查询运行控制详情", "GET", "/api/report/student-runs/{runId}", "runId", "X-User-Id?", "—", "StudentReportRunResponse", "200/404", "Bearer（身份归属）", ""),
    ("报表运行控制", "修改运行控制", "POST", "/api/report/student-runs/{runId}/update", "runId", "X-User-Id?", "StudentReportRunUpdateRequest", "StudentReportRunResponse", "200/400/404", "Bearer（身份归属）", ""),
    ("报表运行控制", "删除运行控制", "POST", "/api/report/student-runs/{runId}/delete", "runId", "X-User-Id?", "—", "{runId,deleted}", "200/404", "Bearer（身份归属）", "逻辑删除"),
    ("报表运行控制", "运行报表创建导出", "POST", "/api/report/student-runs/{runId}/run", "runId", "X-User-Id?", "—", "ExportTaskResponse", "200/404", "Bearer（身份归属）", "businessKey=runId"),
    ("报表运行控制", "查询运行历史任务", "POST", "/api/report/student-runs/{runId}/tasks", "runId", "X-User-Id?", "AsyncTaskPageQueryRequest", "AsyncTaskPageResponse", "200/404", "Bearer（身份归属）", ""),
    ("文件上传中心", "后端上传文件", "POST", "/api/files/upload", "—", "—", "multipart file", "FileUploadResponse", "200/400", "Bearer", ""),
    ("文件上传中心", "秒传检查", "POST", "/api/files/instant-check", "—", "—", "{fileMd5,fileSize}", "InstantUploadCheckResponse", "200/400", "Bearer", ""),
    ("文件上传中心", "初始化直传", "POST", "/api/files/direct/init", "—", "—", "DirectUploadInitRequest", "DirectUploadInitResponse", "200/400", "Bearer", ""),
    ("文件上传中心", "完成直传", "POST", "/api/files/direct/{uploadId}/complete", "uploadId", "—", "—", "FileResponse", "200/404/409", "无", ""),
    ("文件上传中心", "初始化分片上传", "POST", "/api/files/multipart/init", "—", "—", "MultipartUploadInitRequest", "MultipartUploadInitResponse", "200/400", "Bearer", ""),
    ...
    ("文件上传中心", "分页查询文件", "POST", "/api/files/page", "—", "—", "FilePageQueryRequest", "FilePageResponse", "200", "Bearer", ""),
]
