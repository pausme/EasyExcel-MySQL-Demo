...
当前以内部标准测试环境的 R26 轮为最终权威结果（共 26 轮标准环境回归）。R7~R25 保留为历史过程记录，本机和历史云端测试只作为参考，不再作为容量结论。TODO 任务清单 26 项全部完成，代码评审 17 项发现全部处置（见 [docs/code-review.md](docs/code-review.md)）。

| 项目 | 数据量 | 结果 |
| --- | ---: | --- |
| 单元/切片测试 | 30 类 / 176 用例 | 全部通过 |
| 接口扁平化测试 | 77 用例 | 标准环境连续多轮 77/77 通过；F-02/F-09/F-12/F-13 已关闭，F-11 受控（3M 导入实证） |
| 异步导出（XLSX） | 1,000,006 行 × 3 | 全部成功，平均约 23,317 行/s |
| 异步导出（CSV） | 7,000,003 行 × 2 | 全部成功，最高约 85,277 行/s |
| 异步导出（ZIP 分片 CSV） | 7,000,003 行 | 成功，约 90,257 行/s（3M→7M 零衰减） |
| 并发双导出 + 导入 | 2×700 万导出 + 100k 导入同发 | 全部成功；双导出聚合约 92,700 行/s |
| 异步导入 | 100,000 行 × 3 | 全部成功，平均约 4,511 行/s |
| 异步导入 | 1,000,000 行 | 放开护栏后成功，约 4,521 行/s |
| 异步导入 | 3,000,000 行 | 放开护栏后成功（630.5s），约 4,757 行/s，接近线性；全程内存健康 |
...
# 任务执行锁 TTL（默认 3600s）：必须大于预期最长单任务执行时长，否则长任务执行期间锁过期
export TASK_EXECUTION_LOCK_TTL_SECONDS='3600'
...
标准环境最终权威结果（R26，PERF-01 完整矩阵，详见 [docs/performance-report.md](docs/performance-report.md) §5）：

| 场景 | 数据量 | 结果 | 吞吐 |
| --- | ---: | ---: | ---: |
| 服务端播种 | 1,000,000 行（空表） | 88.5 s | 约 11,300 行/s |
| 异步导出（XLSX） | 1,000,006 行，3 次 | 平均 43.05 s | 约 23,317 行/s |
| 异步导出（CSV） | 7,000,003 行，2 次 | 101.5 / 82.1 s | 68,950 / 85,277 行/s |
| 异步导出（ZIP 分片 CSV） | 7,000,003 行 | 77.6 s | 约 90,257 行/s |
| 并发双导出（ZIP+CSV） | 2×7,000,003 行 | 146.2 / 151.2 s | 聚合约 92,700 行/s |
| 异步导入 | 100,000 行，3 次 | 平均 22.42 s | 约 4,511 行/s |
| 异步导入 | 1,000,000 行 | 221.2 s | 约 4,521 行/s |
| 异步导入 | 3,000,000 行 | 630.5 s | 约 4,757 行/s（1M→3M 接近线性） |
...
- 导出 100 万级 XLSX 稳定，超过单 Sheet 上限会按设计失败；**超大数据量使用 `format=ZIP_CSV_PARTS` 或 CSV**，700 万行实测 77~101s 完成，3M→7M 吞吐零衰减。
- 导出线程池 2 与导入 worker 6 已达该云盘瓶颈：双导出并发聚合吞吐与单跑持平，继续加线程无益。
- 300 万导入在放开护栏后接近线性完成且全程内存健康（swap + 分块合并 + 护栏三层缓解生效）。
